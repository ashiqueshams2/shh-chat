...
// Cleanup inactive users/rooms
setInterval(async () => {
  if (chatData.currentUser && chatData.currentRoom) {
    chatData.currentUser.lastSeen = Date.now();
    await database.ref(`users/${chatData.currentUser.id}/lastSeen`).set(Date.now());
  }
  ...
  usersSnapshot.forEach(userSnapshot => {
    ...
    if (now - user.lastSeen > timeout) {
      ...
      roomsRef.once('value', roomsSnapshot => {
        roomsSnapshot.forEach(roomSnapshot => {
          roomSnapshot.child(`users/${user.id}`).ref.remove();
        });
      });
    }
  });
...
window.addEventListener('beforeunload', async () => {
  if (chatData.currentUser && chatData.currentRoom) {
    await database.ref(`rooms/${chatData.currentRoom}/users/${chatData.currentUser.id}`).remove();
    await database.ref(`users/${chatData.currentUser.id}`).remove();
  }
});
